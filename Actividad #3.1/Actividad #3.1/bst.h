...
// =================================================================
// Remove an element, putting a succ node in its place.
//
...
public:
	...
	int depth() const; 
	void currentLevel(Node<T> *, int, std::stringstream &) const; 
	uint leaves() const;
	bool isFull() const;
	T ancestor(T) const;
...
//==================================================================
// Returns currentLevel of the tree 
//
//
//==================================================================
template <class T> 
void BST<T>::currentLevel(Node<T> *root, int level, std::stringstream &aux) const{
	if(root==NULL){
		return ; 
	}
		...
		currentLevel(root->left, level -1, aux);
		currentLevel(root->right, level -1, aux); 
	}
}
